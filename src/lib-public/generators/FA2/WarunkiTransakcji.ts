...
export function generateWarunkiTransakcji(warunkiTransakcji: WarunkiTransakcji | undefined): Content {
  ...
  if (zamowienia.length > 0) {
    const tabZamowienia: TableWithFields = getContentTable<(typeof zamowienia)[0]>(
  ...
  if (partiaTowaru.length > 0) {
    const tabPartiaTowaru: TableWithFields = getContentTable<(typeof partiaTowaru)[0]>(
  ...
  if (warunkiTransakcji.Transport) {
    getTable(warunkiTransakcji.Transport).forEach((transport: Transport, index: number): void => {
      table.push(
